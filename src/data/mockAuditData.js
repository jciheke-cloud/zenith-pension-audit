...
export const INITIAL_AUDIT_PROGRAMS = [
  {
    ...
    procedures: [
      { id: 'p1', step: '1. Sample 50 benefit payment files across Retirement, Death, and Voluntary Contribution categories from Q1 2026.', expectedControl: 'Valid PenCom approval letter and biometric verification slip attached to each file.', evidenceRequired: 'Scanned PenCom approval batch and NIMC/Biometric match log.', status: 'Tested - Pass', findingRef: null },
      { id: 'p2', step: '2. Verify that bank account names match exactly with the RSA beneficiary records without manual override.', expectedControl: 'Automated NIBSS Name Verification API blocks mismatched transfers.', evidenceRequired: 'NIBSS inquiry response logs and core banking exception report.', status: 'Tested - Exception', findingRef: 'FND-2026-003' },
      { id: 'p3', step: '3. Inspect Maker/Checker authorization timestamps in the custodial settlement gateway.', expectedControl: 'Separate Maker and Checker user IDs with minimum 15-minute review interval.', evidenceRequired: 'System audit logs from Custody Settlement Gateway.', status: 'Tested - Pass', findingRef: null }
    ]
  },
  {
    id: 'PROG-CUS-02',
    ...
    procedures: [
      { id: 'p4', step: '1. Perform three-way reconciliation between ZPC General Ledger, PFA Portfolio Valuation, and CSCS/CBN depository statements for Q1.', expectedControl: 'Daily automated reconciliation engine generates zero unexplained variance reports.', evidenceRequired: 'Daily 3-way reconciliation sign-off sheets and variance exception logs.', status: 'Tested - Exception', findingRef: 'FND-2026-001' },
      { id: 'p5', step: '2. Inspect nominee account structure at Central Bank of Nigeria (CBN) for strict PFA asset segregation.', expectedControl: 'Distinct nominee accounts registered per PFA with no pooling of custodian corporate funds.', evidenceRequired: 'CBN confirmation letters and nominee account mandates.', status: 'Tested - Pass', findingRef: null },
      { id: 'p6', step: '3. Review manual journal overrides posted to custodial fee accrual accounts.', expectedControl: 'All manual journals above ₦1,000,000 require Chief Financial Officer sign-off.', evidenceRequired: 'Manual Journal Voucher (MJV) register and approval memos.', status: 'Tested - Exception', findingRef: 'FND-2026-005' }
    ]
  },
  {
    id: 'PROG-SEC-03',
    ...
    procedures: [
      { id: 'p7', step: '1. Extract active database administrator (DBA) accounts and cross-reference against approved HR staff roster.', expectedControl: 'Only active, certified DBAs have sysadmin privileges; former staff revoked within 2 hours.', evidenceRequired: 'Oracle DB user privilege dump and HR separation notifications.', status: 'Tested - Exception', findingRef: 'FND-2026-002' },
      { id: 'p8', step: '2. Verify that multi-factor authentication (MFA) is strictly enforced for VPN and SWIFT Alliance terminal access.', expectedControl: 'Hardware token MFA mandatory for all internal/external administrative sessions.', evidenceRequired: 'Cisco ISE authentication logs and SWIFT security audit trail.', status: 'Tested - Pass', findingRef: null }
    ]
  }
];
